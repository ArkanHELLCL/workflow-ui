...
export default function ListaRegMensajes({frmMessages}){
    ...
    useEffect(() => {
        ...
        /*if(filters.itemIdSelected === 'jr'){
            url = host + '/api/mensajes/bandeja-entrada?PageNumber=1&RowsOfPage=' + filters.maxRecordLoaded
            message = date + ' - Actualizando mensajes de entrada...'
            msgfinal = date + ' - Mensajes de entrada actualizados'
            name = 'usuarios'
            error = false
        }
        if(url){
        ...
        <Suspense fallback={<Loading />}>{
            filteredRequest.length===0 && (
                <div className="text-center flex justify-center lstRequestEmpty align-middle items-center h-[50vh] w-full !overflow-hidden">
                    <span className='text-[#2c87d2] text-xl w-full'>No se encontraron mensajes</span>
                </div>            
            )}
            {filteredRequest.map((item, index) => (
               <RegistroItem registro={item} key={item.id ? 'list-' + item?.id : 'unde-' + index} frmRecord={frmMessages}/>
            ))}
        </Suspense>{
