        ...
        return (      
            <>
                <Button className={`dark:hover:!bg-[#444444] hover:!bg-[#f0f0f0] !bg-transparent !rounded-none dark:!text-stone-100 !text-stone-500 !font-thin !border-none !text-sm !min-h-full !px-2`}
                    id="mnuusr-button"
                    aria-controls={open ? "mnuusr" : undefined}
                    aria-haspopup="true"
                ...
                <Menu 
                    ...
                    onClose={handleClose}
                    MenuListProps={{
                        'aria-labelledby': 'mnuusr-button',
                    }}>
                    <p className="px-6 pt-1 text-xs font-semibold truncate">Filtrar</p>
                    <div className=" max-h-48 overflow-y-auto">{
                        Departamento.records.map((item) =>
                            <MenuItem  className={`hover:!bg-[#c5c5c5] dark:hover:!bg-[#505050] !pr-10 !text-xs !leading-0 !font-normal dark:!text-stone-100 !text-stone-500 !gap-0 !pb-0`} key={item.id} onClick={() => handleSetFDepto(item.id)} id={'itemDepFil-' + item.id}>
                                <ListItemIcon className={``}>{filters.departamento===item.id ? <Check className="!w-4 !h-4" /> : null}</ListItemIcon>{item.shortname}                    
                            </MenuItem> 
                        )
                    }
                    </div>
