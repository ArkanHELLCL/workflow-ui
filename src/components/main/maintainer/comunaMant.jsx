...
export default function MCMant({fields, frmRecord, openDialog, setOpenDialog, mant, record}) {
    ...
    return (
        ...
        <section id="InputsContent" className="py-3 w-full flex flex-col h-full">
            <h2 className='font-base text-lg -mb-1'>Datos del Usuario <span className='text-[#2c87d2]'>Id: {field.COM_Id}</span></h2>
            <h2 className='font-sm text-base -mb-1'>Último editor: <span className='text-[#2c87d2]'>{field.COM_UsuarioEdit}</span></h2>
            <h2 className='font-sm text-base pb-3'>Fecha de edición: <span className='text-[#2c87d2]'>{field.COM_FechaEdit?.slice(0,16)?.replace('T',' ')}</span></h2>
            ...
            <div className="w-full pr-2 flex flex-col overflow-y-auto h-full">            
                ...
                <div className='grid grid-cols-12 gap-2'>
                    <span className='text-[#2c87d2] !text-base !font-normal col-span-12 flex gap-2 items-center uppercase !justify-end'>{parseInt(field.COM_Estado) === 1 && field.COM_Estado !== undefined ?  <ButtonIcon typeButton="btn_habilitar" styles='w-5 h-5'strokeWidth='1.3' typeIcon={1}/> : parseInt(field.COM_Estado) === 1 && field.COM_Estado !== undefined ?<ButtonIcon typeButton="btn_bloquear" styles='w-5 h-5'strokeWidth='1.3' typeIcon={1}/> : null}{parseInt(field.USR_Estado) === 1  && field.COM_Estado !== undefined ? ' Habilitado' : parseInt(field.USR_Estado) !== 1  && field.COM_Estado !== undefined ? ' Deshabilitado' : null}</span>
                </div>                                                
            </div>            
        </section>
        :
