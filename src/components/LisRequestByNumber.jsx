...
export function ListRequestByNumber(min, max, maxAccordions, orderDes, filteredRequest ){    
    ...
    for (let i = 0; i < maxAccordions; i++) {
        ...
        requerimientoAccordion.push({id:i+1, title: title, desde: start, hasta: end, open:true,requerimientos: filteredRequest.filter((req) => req.VRE_Id >= start && req.VRE_Id <= end)})
    }
