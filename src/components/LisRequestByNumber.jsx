...
export function ListRequestByNumber(min, max, maxAccordions, orderDes ){    
    const numReqsPerRange = Math.ceil((max - min + 1) / maxAccordions)
    const requerimientoAccordion = []
    //console.log(numReqsPerRange)
    for (let i = 0; i < maxAccordions; i++) {
        ...
        requerimientoAccordion.push({id:i+1, title: title, diaDesde: start, diaHasta: end})
    }
