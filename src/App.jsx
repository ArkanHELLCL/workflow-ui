...
function App() {
  ...
  return (    
      <div className="dark:bg-[#262626] bg-[#ffffff] z-0 min-h-screen text-sm h-screen w-screen overflow-hidden relative pt-[30px]">        
        <nav className="h-full w-14 absolute left-0 dark:bg-[#363636] bg-[#ffffff] border-r-[1px] border-[#d4d4d4] dark:border-[#484644] transition-color delay-75" onDragOver={handleNotDragOver}>
          <Suspense fallback={<Loading />}>
            <SideBar />
          </Suspense>
        </nav>
        <section className="dark:bg-[#0a0a0a] bg-sky-600 w-full h-[30px] absolute top-0 z-50 transition-color delay-75" onDragOver={handleNotDragOver}>          
          <HeaderBar />          
        </section>
        <Suspense fallback={<Loading />}>
          <Header />
        ...
        <main className='w-full flex overflow-hidden pl-14 h-full pb-[110px]'>
          <section className={`${defaultTheme.txtc} w-[650px] min-w-[400px] h-full flex flex-columns z-0 bg-[#faf9f8] dark:bg-transparent`} id="Resizable" onDragOver={handleNotDragOver}>
